...
export const Route = createFileRoute("/betting/history")({
  component: BettingHistory,
  head: () => ({ meta: [{ title: "Historique des paris" }] }),
});
...
function BettingHistory() {
  ...
  const filtered = useMemo(() => bets.filter((b) =>
    (q === "" || b.id.toLowerCase().includes(q.toLowerCase()) || b.clientId.toLowerCase().includes(q.toLowerCase()) || b.ref.toLowerCase().includes(q.toLowerCase())) &&
      ...
      <Card className="mb-4">
        <CardContent className="p-4 flex flex-wrap items-center gap-3">
          <div className="relative flex-1 min-w-[240px]">
            ...
            <Input placeholder="Rechercher ID, client, référence..." value={q} onChange={(e) => setQ(e.target.value)} className="pl-9" />
          </div>
          <Select value={op} onValueChange={setOp}>
            <SelectTrigger className="w-44"><SelectValue placeholder="Opérateur" /></SelectTrigger>
            <SelectContent>
          ...
          <Select value={gt} onValueChange={setGt}>
            <SelectTrigger className="w-44"><SelectValue placeholder="Type de jeu" /></SelectTrigger>
            <SelectContent>
            ...
            <TableBody>
              {filtered.slice(0, 30).map((b) => (
                <TableRow key={b.id} className="cursor-pointer hover:bg-muted/30">
                  <TableCell className="text-xs text-muted-foreground">{formatDateTime(b.date)}</TableCell>
                  ...
                  <TableCell className="text-right text-sm font-medium">{formatXOF(b.stake)}</TableCell>
                  <TableCell className="text-right text-sm">{formatXOF(b.payout)}</TableCell>
                  <TableCell>
                    <Link to="/betting/details/$id" params={{ id: b.id }}>

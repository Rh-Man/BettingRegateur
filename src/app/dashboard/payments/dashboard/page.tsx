  ...
  return (
    <div>
      <PageHeader
        ...
        description="Activité et performance des prestataires de paiement."
      />

      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
        <KPICard
          title="Transactions totales"
          value={formatNumber(paymentsKpis.totalTransactions)}
          icon={CreditCard}
          change={9.4}
          accent="primary"
        />
        <KPICard
          title="Montant total"
          value={formatXOF(paymentsKpis.totalAmount)}
          icon={Coins}
          change={11.2}
          accent="info"
        />
        <KPICard
          title="Réussies"
          value={formatNumber(paymentsKpis.successful)}
          icon={CheckCircle2}
          change={5.8}
          accent="success"
        />
        <KPICard
          title="Échouées"
          value={formatNumber(paymentsKpis.failed)}
          icon={XCircle}
          change={-2.4}
          accent="destructive"
        />
        <KPICard
          title="Montant moyen"
          value={formatXOF(paymentsKpis.averageAmount)}
          icon={TrendingUp}
          change={3.1}
          accent="info"
        />
        <KPICard
          ...
          value={formatPercent(paymentsKpis.successRate)}
          icon={Percent}
          change={1.2}
          accent="success"
        />
                ...
                {payments.slice(0, 8).map((p) => (
                  <TableRow key={p.id}>
                    <TableCell className="font-mono text-xs">
                      <Link
                        href={`/dashboard/payments/transactions/${p.id}`}
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <OperatorLogo name={p.operator.name} color={p.operator.color} size={20} />
                        <span className="text-sm">{p.operator.name}</span>
                      </div>
                    </TableCell>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle>Volumes par plateforme</CardTitle>
          </CardHeader>
          <CardContent>

      ...
      <div className="grid lg:grid-cols-2 gap-6 mb-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <Row
              label="Paris manquants"
              value={formatNumber(auditSummary.missing.bets)}
              tone="warning"
            />
            <Row
              label="Paiements manquants"
              value={formatNumber(auditSummary.missing.payments)}
              tone="warning"
            />
          </CardContent>
        </Card>
                ...
                <TableRow key={l.id}>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      <OperatorLogo name={l.operator.name} color={l.operator.color} size={20} />
                      <span className="text-sm">{l.operator.name}</span>
                    </div>
                  </TableCell>
...
function Row({ label, value, tone }: { label: string; value: string; tone?: "warning" }) {
  return (
    <div className="flex justify-between items-center p-3 rounded-md bg-muted/40">
      ...
      <span className={`font-semibold ${tone === "warning" ? "text-warning" : ""}`}>{value}</span>
    </div>
  );
}

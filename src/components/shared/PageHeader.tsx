...
import { SlidersHorizontal, ChevronDown, RefreshCw, Filter, Sparkles, CalendarIcon } from "lucide-react";
    ...
    <div className="flex flex-col gap-4 mb-6">
      <div className="flex flex-col items-stretch justify-between gap-4 sm:flex-row sm:items-start">
        <div className="min-w-0">
          <h1 className="text-2xl font-bold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-foreground via-foreground/90 to-foreground/70 sm:text-3xl">{title}</h1>
          {description && <p className="mt-1 text-sm font-medium leading-6 text-muted-foreground">{description}</p>}
        </div>
          ...
          {pageType !== "none" && (
            <Button
              ...
              className={cn(
                ...
                isFiltersOpen ? "bg-primary/10 text-primary border-primary/30" : "glass-card hover:bg-muted"
              )}
            >
      ...
      {pageType !== "none" && (
        ...
        >
          <div className="overflow-hidden">
            <div className="glass-card border-border/50 rounded-2xl p-4 shadow-xl shadow-primary/5 mt-2 relative overflow-hidden sm:p-5">
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4 lg:gap-6">
                ...
                <div className="space-y-2 flex flex-col justify-end">
                  ...
                  <Popover>
                    ...
                    <PopoverContent className="w-auto p-0 rounded-2xl border-border/50 glass-card shadow-2xl" align="start">
                      <Calendar
                        initialFocus
                        mode="range"
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    <Label className="text-xs font-bold text-foreground/80">{config.amountLabel}</Label>
                ...
                {/* Select 1 */}
                <div className="space-y-2 flex flex-col justify-end">
                  <Label className="text-xs font-bold text-foreground/80">{config.select1Label}</Label>
                  <Select value={operator} onValueChange={setOperator}>
                    ...
                    <SelectContent className="rounded-xl border-border/50 glass-card">
                      {config.select1Options.map((opt) => (
                        <SelectItem key={opt.value} value={opt.value} className="font-semibold text-xs rounded-lg">
                          {opt.label}
                        </SelectItem>
                      ))}
                    </SelectContent>
                ...
                {/* Select 2 */}
                <div className="space-y-2 flex flex-col justify-end">
                  <Label className="text-xs font-bold text-foreground/80">{config.select2Label}</Label>
